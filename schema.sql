...
DO $$ BEGIN
    CREATE TYPE product_condition_t  AS ENUM ('high', 'medium', 'low');
...
COMMENT ON COLUMN products.condition   IS 'ENUM product_condition_t: high(상) | medium(중) | low(하)';
...
COMMENT ON COLUMN resale_price_rules.condition
    IS '상품 상태별 가격 규칙 — NULL이면 전체 상태 기본 규칙 (high|medium|low)';
