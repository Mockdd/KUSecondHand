...
function BuyerView({ matchId, packageId, sellerUid, userId, userSemester, userRegionGroup }: BuyerViewProps) {
  ...
  const { data: items = [], isLoading: itemsLoading } = useQuery<CategoryItem[]>({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from('package_items')
        .select('category_id, is_required, requires_hygiene_cert, priority_order, categories(name)')
      ...
      return (data ?? []).map((row) => {
        ...
        return {
          ...
          requires_hygiene_cert: row.requires_hygiene_cert,
  ...
  const { data: listings = [], isLoading: listingsLoading } = useQuery<ListingItem[]>({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        ...
        .select('listing_id, category_id, status, cert_id')
        .eq('match_id', matchId)
  ...
  // ── 셀러 승인된 세탁 인증 ──────────────────────────────────────────────────
  const { data: approvedCertCategoryIds = new Set<number>() } = useQuery<Set<number>>({
    queryKey: ['seller-certs', sellerUid],
    queryFn: async () => {
      const { data, error } = await supabase
      ...
      if (error) throw new Error('세탁 인증 정보를 불러올 수 없어요')
    ...
    <div>
      ...
      <div className="flex flex-col gap-2 mb-6">
        {items.map((item) => {
          ...
          const hasCert = approvedCertCategoryIds.has(item.category_id)
            ...
            >
              <div className="flex items-center justify-between">
                ...
                <div className="flex gap-1">
                  ...
                  {item.requires_hygiene_cert && isOwned && (
                    <span
...
function SellerView({ matchId, packageId, userId }: SellerViewProps) {
  ...
  // ── 세탁 인증 현황 ─────────────────────────────────────────────────────────
  const { data: certCategoryIds = new Set<number>() } = useQuery<Set<number>>({
    queryKey: ['seller-certs', userId],
    queryFn: async () => {
      const { data, error } = await supabase
        .from('hygiene_certifications')
        .select('category_id')
  ...
  const hygieneRequiredIds = new Set(
    items.filter((i) => i.requires_hygiene_cert).map((i) => i.category_id)
  )
  const isPostable =
    myListings.length > 0 &&
    [...hygieneRequiredIds].every(
      (cid) => ownedCategoryIds.has(cid) && certCategoryIds.has(cid)
    )
  ...
  return (
    <div>
      ...
      <div className="mb-4">
        ...
        >
          {isPostable ? '게시 가능 — 세탁 인증 완료' : '게시 불가 — 세탁 인증 미완료'}
        </span>
      </div>
      ...
      <div className="flex flex-col gap-2 mb-6">
        {items.map((item) => {
          const isOwned = ownedCategoryIds.has(item.category_id)
          const hasCert = certCategoryIds.has(item.category_id)
      ...
      {!isPostable && (
        <Link
          href="/sell/hygiene"
          className="block text-center px-4 py-2 border border-gray-300 text-sm rounded mb-6"
        >
          세탁 인증 등록하기
        </Link>
      )}
...
export default function PackageDetailPage() {
  ...
  const {
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from('package_matches')
        .select('match_id, package_id, seller_uid, status, semester')
        .eq('match_id', matchId)
        .single()
  ...
  // ── 패키지 이름 조회 ───────────────────────────────────────────────────────
  const { data: packageInfo } = useQuery<{ name: { ko: string; en: string } } | null>({
    ...
    queryFn: async () => {
      ...
      if (error) return null
      return data as { name: { ko: string; en: string } }
    },
  })
  ...
  const packageName = packageInfo?.name?.ko ?? '패키지'
  ...
  return (
    <div className="p-8">
      ...
      <div className="flex items-center gap-3 mb-6">
        <Link href="/packages" className="text-sm text-gray-500 underline">
          ← 목록
        </Link>
        <div>
