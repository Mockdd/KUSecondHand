    ...
    queryFn: async (): Promise<ChatListItem[]> => {
      ...
      return data
        .map((m) => {
          ...
          const packageNameRaw = Array.isArray(m.essential_packages)
            ? (m.essential_packages[0] as { name: { ko: string; en: string } } | undefined)?.name
            : (m.essential_packages as { name: { ko: string; en: string } } | null)?.name

          const packageName =
            packageNameRaw?.ko ?? `패키지 #${m.package_id}`
